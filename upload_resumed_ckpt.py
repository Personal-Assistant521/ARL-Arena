...
# 下载检查点 - 自动方式（推荐，会自动选择最佳方法）
'''
hf download willamazon1/Qwen3-4B-search-agent-checkpoint
'''

# 上传检查点 - 快速方式（直接上传文件，不创建tar包）- 现在默认为公开仓库
'''
python upload_resumed_ckpt.py upload \
  --checkpoint_dir "./Qwen3-4B_gigpo_len_450_hybrid_stop/global_step_5/actor" \
  --repo_name "willamazon1/Qwen3-4B-search-agent-checkpoint" \
  --token "xxx" \
  --no-archive
'''
