...
class AdvantageEstimator(str, Enum):
    ...
    SAMPO = "sampo"  # Sequence-level Adaptive Policy Optimization
...
class ShopAgentTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.SAMPO:
            with open_dict(self.config):
                self.config.actor_rollout_ref.actor.policy_loss.loss_mode = "gspo"
                self.config.algorithm.adv_estimator = AdvantageEstimator.GiGPO
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw):
                            ...
                            print(f"self.config.actor_rollout_ref.actor.loss_agg_mode: {self.config.actor_rollout_ref.actor.loss_agg_mode}",'*'*100)
