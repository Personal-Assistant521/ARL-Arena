    ...
    for i in range(batch.batch.batch_size[0]):
        ...
        if turn_idx+1 < len(steps2traj_f_H[traj_uid]):
            try:
                ...
                print(f"Error: {turn_idx + 1} is out of range for traj_uid: {traj_uid}")
                print(f"steps2traj_f_H[traj_uid]: {steps2traj_f_H[traj_uid]}")
                print(f"traj_uid: {traj_uid}")
                print(f"turn_idx: {turn_idx}")
                print(f"len(steps2traj_f_H[traj_uid]): {len(steps2traj_f_H[traj_uid])}")
                raise ValueError(f"Error: {turn_idx + 1} is out of range for traj_uid: {traj_uid}")
