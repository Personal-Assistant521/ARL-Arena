#!/usr/bin/env bash
# One-stop runner for shop-agent API evaluation.
# Usage (override vars if needed):
#   MODEL="gpt-4o-mini" TRAIN_DATA=~/data/text/train.parquet VAL_DATA=~/data/text/test.parquet bash recipe/shop_agent/eval_shop_agent.sh
...
export RAY_TMPDIR="/data2/ray_out"
rm -rf "$RAY_TMPDIR"
mkdir -p "$RAY_TMPDIR"
